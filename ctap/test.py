...
def getPublicCredOptions(challenge_data, rp_id):
    ...
    result["user"] = PublicKeyCredentialUserEntity(challenge_data["user"]["id"].encode(),
                                                   challenge_data["user"]["name"])
...
def getAssertionData(data, rp_id):
    for i, value in enumerate(data["allowCredentials"]):
        data["allowCredentials"][i]["id"] = base64.urlsafe_b64decode(data["allowCredentials"][i]["id"])
