...
D = DomainName('app1.com.')
D2 = DomainName('app2.com.')
IP = '192.168.1.33'
TTL = 60 * 5
...
records2 = {
    D2: [A(IP), AAAA((0,) * 16), MX(D2.mail), soa_record] + ns_records2,
...
def dns_response(data):
    ...
    if qn == D2 or qn.endswith('.' + D2):
        for name, rrs in records2.items():
            if name == qn:
                for rdata in rrs:
                    rqt = rdata.__class__.__name__
                    if qt in ['*', rqt]:
        ...
        for rdata in ns_records2:
            reply.add_ar(RR(rname=D2, rtype=QTYPE.NS, rclass=1, ttl=TTL, rdata=rdata))

        reply.add_auth(RR(rname=D2, rtype=QTYPE.SOA, rclass=1, ttl=TTL, rdata=soa_record2))
